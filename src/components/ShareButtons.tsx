...
export function ShareButtons({ title, slug, image }: ShareButtonsProps) {
  ...
  const shareToInstagramStory = async () => {
    ...
    try {
      ...
      // Check if on mobile with Instagram app support
      ...
      // If all fails, just copy the link
      await navigator.clipboard.writeText(`${title}\n\n${url}`);
      setStatus("success");
      setTimeout(() => setStatus("idle"), 2000);
    }
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        ...
        className="p-3 rounded-full bg-[#25D366] text-white hover:bg-[#20bd5a] transition-colors"
      ...
      <button
        ...
        title={status === "success" ? "Pronto!" : "Compartilhar no Instagram Story"}
      >
        {status === "loading" ? (
